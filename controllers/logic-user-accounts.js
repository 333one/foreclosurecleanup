...
exports.assembleCompanyServices = function (boardingSecuring, debrisRemovalTrashout, evictionManagement, fieldInspection, handymanGeneralMaintenance, landscapeMaintenance, lockChanges, overseePropertyRehabilitation, poolMaintenance, propertyCleaning, winterizations) {
	let propertyObject = {
		boardingSecuring,
...
exports.formatCompanyDescription = function (companyDescription) {
	...
	let isPunctuationAtEndOfCompanyDescription = companyDescription.charAt(companyDescriptionLength - 1) === '.' || companyDescription.charAt(companyDescriptionLength - 1) === '!' || companyDescription.charAt(companyDescriptionLength - 1) === '?' ? true : false;
...
exports.testFormattedURLAndSave = async function (theURL, email) {
	let httpProtocol = 'http://';
	let httpsProtocol = 'https://';

	let doesNormalizedURLStartWithHttp = theURL.startsWith(httpProtocol) === true ? true : false;
	let doesNormalizedURLStartWithHttps = theURL.startsWith(httpsProtocol) === true ? true : false;

	let httpURL, httpsURL;

	if (doesNormalizedURLStartWithHttp === true) {
		httpURL = theURL;
		httpsURL = httpsProtocol + theURL.slice(7);
	}

	if (doesNormalizedURLStartWithHttps === true) {
		httpsURL = theURL;
		httpURL = httpProtocol + theURL.slice(8);
	}

	let isHttpURLActive, isHttpsURLActive;

	await Promise.allSettled([httpCheck(httpURL), httpsCheck(httpsURL)]).then((responses) => {
		isHttpURLActive = processAxiosResponse(responses[0]);
		isHttpsURLActive = processAxiosResponse(responses[1]);
	});
	...
	if (user.companyWebsite !== '') {
		...
		if (isHttpURLActive === false && isHttpsURLActive === false) {
			...
			let emailBody = emailMessage.urlNotActiveBody(theURL);
			communication.sendEmail(siteValue.noReplyEmail, email, emailSubject, emailBody);
		} else if (isHttpsURLActive === true && httpsURL !== user.companyWebsite) {
			await User.updateOne({ email }, { companyWebsite: httpsURL });
		} else if (isHttpURLActive === true && httpURL !== user.companyWebsite) {
			await User.updateOne({ email }, { companyWebsite: httpURL });
		}
	}
};

function axiosGet(theURL) {
...
function httpCheck(httpURL) {
	return axiosGet(httpURL);
}

function httpsCheck(httpsURL) {
	return axiosGet(httpsURL);
}

function processAxiosResponse(response) {
	if (response.status === 'rejected') return false;

	let doesResponseStartWith2 = String(response.value.status).startsWith('2');
	if (doesResponseStartWith2 === true) return true;
