...
exports.checkAreAllAccountPropertiesFilled = function (sessionUserValues, currentProperty) {
	...
	if (currentProperty === 'companyAddress') {
		if (userProperties.companyName === '' || userProperties.companyPhone === '') return false;
	} else if (currentProperty === 'companyName') {
		if (userProperties.companyPhone === '' || userProperties.companyCity === '' || userProperties.companyState === '' || userProperties.companyStreet === '' || userProperties.companyZipCode === '') return false;
	} else if (currentProperty === 'companyPhone') {
		if (userProperties.companyName === '' || userProperties.companyCity === '' || userProperties.companyState === '' || userProperties.companyStreet === '' || userProperties.companyZipCode === '') return false;
	} else if (currentProperty === 'companyServices') {
		if (userProperties.companyName === '' || userProperties.companyPhone === '' || userProperties.companyCity === '' || userProperties.companyState === '' || userProperties.companyStreet === '' || userProperties.companyZipCode === '') return false;
	}
...
exports.checkDoAnyCompanyServicesHaveValue = function (boardingSecuring, debrisRemovalTrashout, evictionManagement, fieldInspection, handymanGeneralMaintenance, landscapeMaintenance, lockChanges, overseePropertyRehabilitation, poolMaintenance, propertyCleaning, winterizations) {
...
exports.checkIfCompanyAddressNormalized = function (companyStreet, companyStreetTwo, companyCity, companyState, companyZip, normalizedCompanyAddress) {
	if (normalizedCompanyAddress.street1 === companyStreet && normalizedCompanyAddress.street2 === companyStreetTwo && normalizedCompanyAddress.city === companyCity && normalizedCompanyAddress.state === companyState && normalizedCompanyAddress.Zip5 === companyZip) return true;
...
exports.checkIfVendorLogoReqFieldsValid = function (reqBody, reqFile, addChangeCompanyLogo) {
	...
	if (isNumberOfBodyKeysValid === true && isNumberOfFileKeysValid === true && isTextFieldNameValid === true && isImageFieldNameValid === true) {
		return true;
	} else {
		return false;
	}
...
exports.checkIfURLIncludedProtocol = function (companyWebsiteLowercase) {};
