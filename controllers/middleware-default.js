...
exports.foreclosureCleanupVendorList = wrapAsync(async function (req, res) {
	...
	if (searchType === 'new' || searchType === 'multiple') {
		let cleanedQuery = submissionProcessing.cleanQuery(
			formFields.foreclosureCleanupVendorList,
			req.query
		);
		let cleanedQueryWithBoolean = logicDefault.convertStringToBoolean(
			cleanedQuery,
			defaultValue.listOfCompanyServices
		);
		...
		searchRadiusAttributesAndValue = logicDefault.createSearchRadiusAttributesAndValue(
			defaultValue.searchRadius,
			searchRadiusNumerical
		);
		servicesAttributesAndValues = logicDefault.createServicesAttributesAndValues(
			defaultValue.listOfCompanyServices,
			cleanedQueryWithBoolean
		);
		...
		let isSearchZipCodeValidCharacters = regSearchZipCode.test(
			searchZipCode
		);
		let isSearchZipCodeFiveDigits =
			searchZipCode.length === 5 ? true : false;

		// Don't search for zip code lat and long if there is an error.
		if (
			isSearchZipCodeFilled === true &&
			isSearchZipCodeValidCharacters === true &&
			isSearchZipCodeFiveDigits === true
		) {
			let geoLocationOfZip = await client.geocodeForward(searchZipCode, {
				autocomplete: false,
				limit: 3,
				types: 'postcode'
			});
			...
			if (geoLocationOfZip.entity.features.length > 0) {
				let geoLocationAnswerLatLong = logicDefault.getGeoLocationAnswerLatLong(
					searchZipCode,
					geoLocationOfZip
				);
				var {
					...
					zipCodeLongAndLat
				} = geoLocationAnswerLatLong;
			}
		}

		let isSearchRadiusFilled = searchRadius === '' ? false : true;
		let isSearchRadiusValid = defaultValue.searchRadius.includes(
			searchRadiusNumerical
		);

		let searchServicesForChecking = logicDefault.createSearchServicesForChecking(
			cleanedQueryWithBoolean,
			defaultValue.listOfCompanyServices
		);
		let isAtLeastOneServiceChecked = checks.checkIfAtLeastOneCompanyServiceFilled(
			searchServicesForChecking
		);
		let wereAllServiceValuesValid = checks.checkIfServiceValuesValid(
			searchServicesForChecking
		);
		...
		let privacyTermsBoolean = submissionProcessing.convertCheckboxToBoolean(
			privacyTerms
		);
		...
		if (
			...
			let searchResults = await User.find(mongooseSearchObject)
				.select(mongooseSelectObject)
				.lean();

			let freeAccounts = logicDefault.separateAccountTypes(
				searchResults,
				defaultValue.accountDefault
			);
			let premiumAccounts = logicDefault.separateAccountTypes(
				searchResults,
				defaultValue.accountUpgrade
			);

			let freeAccountsProcessed = logicDefault.processCompanyForViewing(
				freeAccounts
			);
			let premiumAccountsProcessed = logicDefault.processCompanyForViewing(
				premiumAccounts,
				defaultValue.accountUpgrade
			);

			let searchAgainNamesValues = logicDefault.getSearchAgainNamesValues(
				cleanedQueryWithBoolean
			);
			...
			let searchOutput = {};
			searchOutput.free =
				Array.isArray(freeAccountsProcessed) === true ? true : false;
			searchOutput.premium =
				Array.isArray(premiumAccountsProcessed) === true ? true : false;
			...
			let searchZipCodeError = errorMessage.getSearchZipCodeError(
				isSearchZipCodeFilled,
				isSearchZipCodeValidCharacters,
				isSearchZipCodeFiveDigits,
				isZipCodeRealAndInUsa
			);
			let searchRadiusError = errorMessage.getSearchRadiusError(
				isSearchRadiusFilled,
				isSearchRadiusValid
			);
			let searchServicesError = errorMessage.getSearchServicesError(
				isAtLeastOneServiceChecked,
				wereAllServiceValuesValid
			);

			// Only grab this error on new searches, not a "search again".
			if (searchType !== 'multiple') {
				var privacyTermsError = errorMessage.getPrivacyTermsError(
