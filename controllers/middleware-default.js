...
exports.foreclosureCleanupVendorList = wrapAsync(async function (req, res) {
	...
	if (searchType === 'new' || searchType === 'multiple') {
		...
		let { searchRadius, searchZipCode, boardingSecuring, debrisRemovalTrashout, evictionManagement, fieldInspection, handymanGeneralMaintenance, landscapeMaintenance, lockChanges, overseePropertyRehabilitation, poolMaintenance, propertyCleaning, winterizations } = cleanedQueryWithBoolean;
		...
		if (isSearchZipCodeFilled === true && isSearchZipCodeValidCharacters === true && isZipCodeRealAndInUsa === true && isSearchRadiusFilled === true && isSearchRadiusValid === true && isAtLeastOneServiceChecked === true && wereAllServiceValuesValid === true && isPrivacyTermsChecked === true) {
			let mongooseSearchObject = mongooseLogic.createMongooseSearchObject(searchRadiusNumerical, zipCodeLongAndLat, {
				boardingSecuring,
...
exports.postForeclosureCleanupVendorList = wrapAsync(async function (req, res) {
	...
	let { searchRadius = '', searchZipCode = '', boardingSecuring = 'false', debrisRemovalTrashout = 'false', evictionManagement = 'false', fieldInspection = 'false', handymanGeneralMaintenance = 'false', landscapeMaintenance = 'false', lockChanges = 'false', overseePropertyRehabilitation = 'false', poolMaintenance = 'false', propertyCleaning = 'false', winterizations = 'false', privacyTerms = 'notChecked', searchType = '' } = req.body;
